        ...
        <div class="row justify-content-center py-5">
            <div class="col-12 col-lg-6">
                <div class="bg-white rounded-4 shadow p-4 p-md-5">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold text-main-blue" for="abstractName"><?= lang('abstract__fields.name') ?></label>
                        <input class="form-control" id="abstractName" name="name" type="text"
                           data-validate="required|string|min:3|max:100" value="<?= old('name') ?>" required placeholder="<?= lang('abstract__placeholders.name') ?>">
                        <?= errors('name', session('errors')) ?>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-main-blue" for="abstractEmail"><?= lang('abstract__fields.email') ?></label>
                        <input class="form-control" id="abstractEmail" name="email" type="email"
                            data-validate="required|email|max:255"
                            value="<?= old('email') ?>" required placeholder="<?= lang('abstract__placeholders.email') ?>">
                        <?= errors('email', session('errors')) ?>
                    </div>

                    <div class="mb-2">
                        <label class="form-label fw-semibold text-main-blue" for="abstractFile"><?= lang('abstract__fields.file') ?></label>
